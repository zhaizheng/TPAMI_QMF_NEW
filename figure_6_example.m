...
for j = 1:length(Delta)
    [f, resl] = fit_nonlinear(train, Tau, rho, Delta(j), 1);
    Rho = [Rho, resl.rho];
    S = [S, resl.t];
    P{j} = [];
    ...
    error(j) = norm(P{j}-truth,'fro');
    A = 1:50;
    if j==1 || j==length(Delta) 
        nexttile
...
function [f, resl] = fit_nonlinear(Data, Tau, rho, delta, adaptive, W)
    ...
    while true
        ...
        [c, A, P] = Least_Fitting(Data, Tau, resl.rho, W);
        d = size(Tau, 1);
        B = build_tensor(P, d);
