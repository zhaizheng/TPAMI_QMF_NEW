...
for i = 1:size(data,2)
    ...
    W = eye(size(Data_p,2));
    [data_new_temp, Taus, iter, Error, q_p, rho] = quadratic_fit(Data_p, q, Data_p, Tau_p, h, center, W);
    data_new{i} = data_new_temp; 
    P = [P, q_p(:,end)]; 
    Rho(i) = rho;
    i
end
o1 = sqrt(norm(data-data_t,'fro')^2/300);
o2 = sqrt(norm(P-data_t,'fro')^2/300);
