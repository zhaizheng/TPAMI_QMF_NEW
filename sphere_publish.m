...
for i = 1:3
    ...
    Re_MSE(i) = norm(A,'fro')^2/size(data,2);
    Re_STD(i) = std(sum(A.^2,1),1);
    fprintf('MSE%d=%.3f,STD:%.3f, rho=%.3f\n',i,Re_MSE(i),Re_STD(i),mean(Rho{i}));
    set(gca,'FontSize',18)
end
str{4} = 'original';
...
legend(str)
%title('Fitting Error for Each $x_i$','FontSize',18,'Interpreter','latex')
xlabel('$$i$$','Interpreter','latex')
ylabel('$$\|\hat{x}_i-P_{\cal S}(\hat{x}_i)\|_2^2$$','Interpreter','latex')
%%
% figure
