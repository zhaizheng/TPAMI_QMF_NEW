...
plot(Rho, error,'*-','MarkerSize',8,'Linewidth',2)
ylabel('MSE')
%xlabel('\lambda')
...
hold on
plot(convert(Delta), error,'o-','MarkerSize',8,'Linewidth',2)
...
hold on
plot(convert(S), error,'>-','MarkerSize',8,'Linewidth',2)
%xlabel('s(\lambda)')
%axis([min(S),max(S),min(error),max(error)])
set(gca,'FontSize',18)
legend({'\lambda','-s''(\lambda)','s(\lambda)'})


function b = convert(a)
    b = (a - min(a))/(max(a)-min(a))*0.1;
end
