...
for j = 1:2
 ...
 %   subplot('position', [0.02*(j+1)+0.22*j 0.12 0.22 0.8]);
    ...
    for i = 1:size(data,2)
        ...
        elseif j==2
            rho = mean(Rho{1});
            [f,rho] = fit_nonlinear(Data_p, Tau_p, rho, delta, 0);
        end
        Rho{j} = [Rho{j},rho];
